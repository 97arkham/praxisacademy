...
export default {
  data: () => {
    return {
      ...
      // validate
      nameVal: [(v) => v.length >= 3 || "Huruf Harus Lebih Dari 3 Huruf"],
      programVal: [(v) => !!v || "Pilih Salah Satu Program"],
      emailVal: [
        (v) => !!v || "Masukan Email",
        (v) => /.+@.+\..+/.test(v) || "Email Salah Penulisan",
      ],
      tempatlahirVal: [(v) => !!v || "Masukan Tempat Lahir"],
      kotaasalVal: [(v) => !!v || "Masukan Kota Asal"],
      status: [(v) => !!v || "Masukan Status"],
      nomorhpVal: [(v) => !!v || "Masukan Nomor Telepon"],
      alamatVal: [(v) => !!v || "Masukan Alamat"],
      pendidikanVal: [(v) => !!v || "Masukan Pendidikan"],
      namakampusVal: [(v) => !!v || "Masukan Nama Kampus"],
      alamatkampusVal: [(v) => !!v || "Masukan Alamat Kampus"],
      alasanikutVal: [(v) => !!v || "Masukan Alasan Ikut"],
      menyelesaikanVal: [(v) => !!v || "Pilih Komitmen"],
      referensiVal: [(v) => !!v || "Pilih Referensi"],
      mediasosialVal: [(v) => !!v || "Masukan Media Sosial"],
